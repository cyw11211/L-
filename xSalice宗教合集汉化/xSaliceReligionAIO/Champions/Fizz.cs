    ...
    class Fizz : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            var spellMenu = new Menu("技能", "SpellMenu");
            {
                ...
                {
                    qMenu.AddItem(new MenuItem("Q_Min_Dist", "最小距离使用E", true).SetValue(new Slider(100, 1, 475)));
                    spellMenu.AddSubMenu(qMenu);
                }

                var eMenu = new Menu("E菜单", "EMenu");
                {
                    eMenu.AddItem(new MenuItem("E_Min_Dist", "最小距离使用E", true).SetValue(new Slider(100, 1, 400)));
                    spellMenu.AddSubMenu(eMenu);
                }

                var rMenu = new Menu("R菜单", "RMenu");
                {   
                    rMenu.AddItem(new MenuItem("rBestTarget", "R最佳的目标", true).SetValue(new KeyBind("R".ToCharArray()[0], KeyBindType.Press)));
                    ...
                    rMenu.AddItem(new MenuItem("ROverkill", "R击杀检测", true).SetValue(false));
                    rMenu.AddItem(new MenuItem("AlwaysR", "连招总是使用R", true).SetValue(true));
            ...
            var farm = new Menu("清兵", "Farming");
            {
                ...
                farm.AddItem(new MenuItem("LaneClear_useE_minHit", "使用E命中", true).SetValue(new Slider(2, 1, 6)));
                //add to menu
            ...
            var drawMenu = new Menu("显示", "Drawing");
            {
                ...
                drawMenu.AddItem(new MenuItem("Draw_Mode", "显示模式", true).SetValue(true));
        ...
        private void ModeSwitch()
        {
            ...
            if (menu.Item("Combo_Switch", true).GetValue<KeyBind>().Active && lasttime > Game.Ping)
            {
                if (mode == 0)
                {
                    menu.Item("Combo_mode", true).SetValue(new StringList(new[] { "R-W-Q-E (正常)", "W-Q-R-E(R 期间 Q)", "R-E-W-Q (R->E 突进)" }, 1));
                    _lasttick = Environment.TickCount + 300;
                }
                else if (mode == 1)
                {
                    menu.Item("Combo_mode", true).SetValue(new StringList(new[] { "R-W-Q-E (正常)", "W-Q-R-E(R 期间 Q)", "R-E-W-Q (R->E 突进)" }, 2));
                    _lasttick = Environment.TickCount + 300;
                }
                else
                {
