    ...
    class Katarina : Champion
    {
        ...
        private void LoadMenu()
        {
            var key = new Menu("按键", "Key");{
                ...
                key.AddItem(new MenuItem("Wardjump", "逃跑/跳眼",true).SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Press)));
                //add to menu
                menu.AddSubMenu(key);
            }
            ...
            var farm = new Menu("补兵", "Farm");
            {
                ...
                farm.AddItem(new MenuItem("UseQHit", "使用 Q 补兵", true).SetValue(false));
            ...
            {
                killSteal.AddItem(new MenuItem("smartKS", "使用智能击杀系统", true).SetValue(true));
